...
| --- | --- |
| ESLint | Pass |
| Deterministic tests | **341/341 pass; 0 skipped or failed** |
| `git diff --check` | Pass |
| Expo Doctor | **20/20** |
...
- All 155 JSON catalogs embed the same 353 non-empty keys: 319 established interface keys and 34 `releaseCritical` keys. `testflight-full-catalog` is a separately named, production-like signed layout/coverage audit that enables all 155 without a code change; preview, base TestFlight, and production remain restricted.
- General per-string fallback remains disabled. The six pre-existing critical blocks for `en/es/fr/zh/ar/he` were preserved. The other 149 were machine-generated by Codex `gpt-5.6-sol` on 15 July 2026 and are loaded from their selected catalogs with no English overlay fallback. They remain pending native-speaker safety review, so structural completeness is not linguistic approval.
...
- Both TestFlight profiles use store distribution, production-like readiness checks, real-device archives, and auto-incremented build numbers. Only `testflight-full-catalog` expands the catalog surface, and review-required language choices remain marked `QA` in the picker/trigger.
...
| RTL/responsive | Base profile: `en/es/fr/zh/ar/he` with repeated LTR/RTL relaunch. Full-catalog profile: 156 picker rows; German/Portuguese/Russian plus representative additional RTL such as Urdu; review-required picker/trigger `QA` badges and selected-language critical copy with no English overlay fallback. Both: iPhone SE-class, current Pro-class, iPad portrait/landscape/split view, Dynamic Type, VoiceOver, keyboard. |
...
| P1 | Full-catalog critical-copy linguistic approval | The 149 Codex `gpt-5.6-sol` critical blocks are structurally complete but machine-generated; native safety reviewers must approve them before any affected locale enters public production. |
