...
| --- | --- | --- |
| `EXPO_PUBLIC_API_BASE_URL` | HTTPS root used by provider-token exchange, phone verification, safety contacts/sessions/SOS, privacy, and authenticated CLM control requests. | Set after deploying and verifying the repository's HTTP service, for example the Vercel HTTP root or a reviewed single-container Railway root. Default: empty in the template; the current ignored local `.env` uses the isolated Railway staging HTTPS root. |
...
| `EXPO_PUBLIC_HUME_WS_PROXY_URL` | TLS WebSocket endpoint for live PCM voice, normally `wss://<voice-host>/api/voice/hume-ws`. | URL of the separately deployed long-lived Node container after authenticated Hume, security/load, and signed-device tests pass. The Vercel HTTP adapter does not host this raw WebSocket route, and a Railway liveness deployment alone is not an approved value. Default: empty. |
...
| `EXPO_PUBLIC_SHOW_ALL_LANGUAGES` | Exposes all 155 structurally complete, 353-key JSON catalogs for layout/coverage audit. The 149 newly added critical blocks are Codex `gpt-5.6-sol` machine translations dated 15 July 2026 and require native safety review; no English critical overlay fallback is used. | Default: `false`. Set `true` only with development metadata or use the committed `testflight-full-catalog` profile; base TestFlight, preview, and production keep it `false`. |
...
For Railway, connect the repository root so the committed `railway.toml` is applied. It selects `server/Dockerfile`, watches `server/**` and `railway.toml`, gates deployment on `/health` for 60 seconds, and restarts on failure up to three times; no `RAILWAY_DOCKERFILE_PATH` variable is needed. Configure environment-scoped secrets, generated/custom domains, ingress/path rules, WebSocket idle/frame/connection limits, alerts, and rollback separately. The current staging container has also completed a synthetic first-party session through to Hume `auth_ok`, so the ignored local development `.env` may use its WSS path. Do not promote that value to EAS/production until signed-provider PCM, security/load, and rollback evidence pass.
...
1. Obtain the organization API key from the [Hume developer portal](https://app.hume.ai/developers). Follow Hume's [API key guide](https://dev.hume.ai/docs/introduction/api-key). This implementation consumes `HUME_API_KEY`; put it only in the backend secret manager and never in the production mobile environment. It does not consume a separate Hume secret-key variable.
...
- `testflight-full-catalog` is a separately identifiable signed layout/coverage audit and exposes all 155 JSON catalogs plus System default. It is production-like for credentials, transports, entitlements, and readiness checks. Every catalog embeds all 353 keys with no English critical overlay fallback, but the 149 Codex `gpt-5.6-sol` blocks remain machine-generated and unreviewed, so this artifact is not native-language acceptance.
