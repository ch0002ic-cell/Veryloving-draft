...
- [ ] Confirm availability matches the recorded profile: base `testflight` has exactly **System default, English, Spanish, French, Simplified Chinese, Arabic, and Hebrew** (7 rows); `testflight-full-catalog` has System default plus all 155 catalogs (156 rows), with `QA` badges on the 151 review-required rows. A mismatch is a profile/build defect.
...
- [ ] Exercise auth, map/share, SOS, BLE, voice, contacts, and Saved Places errors. Confirm the 34 critical strings render from the selected catalog rather than an English overlay fallback; no raw missing-key diagnostic is acceptable. Return to Settings and confirm a review-required language remains marked `QA`.
- [ ] Record this artifact as structural/layout audit only. All 155 JSON catalogs embed all 353 keys. The six pre-existing critical blocks were preserved; the 149 blocks generated by Codex `gpt-5.6-sol` on 15 July 2026 still require native-speaker safety review.
...
- [ ] For a full-catalog build: screenshots of the picker/trigger `QA` badge with one additional LTR locale and one additional RTL locale, plus representative embedded critical copy in each selected language.
