...
- **Authentication:** Apple and Google identity assertions now go to `POST /v1/auth/exchange`; phone sign-in uses Twilio Verify through signed five-minute `/v1/auth/phone/start` and `/v1/auth/phone/verify` challenges. The server issues distinct access/refresh JWTs with opaque phone subjects, and the app has no fixed-code or fabricated-token path. `POST /v1/auth/refresh` rotates the client-held pair; the app renews before expiry/on cold start/foreground, preserves offline state for network failures, and clears rejected sessions. Persistent refresh-family state, reuse detection/revocation, deletion tombstones, provider credential-state checks, distributed abuse controls, production credential deployment, and uniform request-level 401 retry remain open.
...
On 14 July 2026, the auth/Vercel/animation/entitlement follow-up passed ESLint, 215/215 tests, Expo Doctor 20/20, and both production exports. The app-owned Safety Call pulse was moved off React Native's legacy `NativeAnimated` emitter. A clean Debug build installed on the iOS 26.5 simulator completed cold launch, onboarding/account transitions, and an isolated active-indicator probe with no `onAnimatedValueUpdate` entry in the simulator log archive. Notification and SecureStore preflights were then extended so unsupported simulator/Expo Go hosts never evaluate the entitlement-dependent package path. A buffer-clean cold launch logged the two intentional fallback notices once and contained none of the historical Dev Launcher `sharedPackageConnection`, notification-registration Keychain, or Auth SecureStore warning signatures. This closes the app-owned warning regressions, but it is not an authenticated end-to-end Safety Call or physical-device result.
...
4. Deploy/configure Twilio Verify with geo/fraud/rate-limit policy and distributed API abuse controls; connect push registration/delivery, live map/share/route endpoints, and idempotent safety-mode transitions.
