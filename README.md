...
Expo Go is a UI and foreground-flow preview only. VeryLoving skips its notification integration there—including local scheduling—because evaluating the SDK 57 package root starts an entitlement-dependent push-registration Keychain read. Notifications, Google Sign-In, production Apple identity exchange, Mapbox, BLE, and background audio require a development or signed build. Apple Authentication can open in Expo Go, but its Expo Go-scoped identity is not production validation. To avoid host-Keychain entitlement warnings, Expo Go never loads SecureStore and uses volatile process memory instead; its session will not survive a JavaScript reload or app restart. Development and signed builds continue to use native SecureStore and fail closed instead of downgrading security. Expo Go results cannot close native launch gates.
