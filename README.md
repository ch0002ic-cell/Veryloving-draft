...
| `EXPO_PUBLIC_SHOW_ALL_LANGUAGES` | Exposes all 155 structurally complete, 353-key JSON catalogs for layout/coverage auditing. | `true` only with development metadata or the committed `testflight-full-catalog` profile; preview, base TestFlight, and production remain false. The 149 Codex `gpt-5.6-sol` critical blocks remain machine-generated and require native safety review; no English critical overlay fallback is used. |
...
- native-speaker safety-copy review for Arabic and Hebrew before either base-TestFlight QA locale enters public production; the other 149 catalog work products remain nonselectable outside development and the separately marked `testflight-full-catalog` layout-audit profile, and their complete but machine-generated Codex `gpt-5.6-sol` critical blocks still require native safety review.
